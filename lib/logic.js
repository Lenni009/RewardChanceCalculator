function parseXML(inputId, outputId) {
	...
	for (let i = 0; i < IDs.length; i++) {
		const itemId = IDs[i];
		const chance = (xmlDoc.querySelector('[name="RewardChoice"]').getAttribute("value") == 'GiveAll') ? parseFloat(chances[i]) : (chances[i] / (chances.reduce((a, b) => parseFloat(a) + parseFloat(b), 0) / 100));
